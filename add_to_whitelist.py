...
class SteamGroup:
    ...
    """
    XML_URL = 'http://steamcommunity.com/groups/bot-tf-bots' + '/memberslistxml?xml=1' # Add your custom group URL here
    XML_NEXT_PAGE = False
    XML_PAGE = 1
...
bot_tf_ids = steamgroup.get_steam_ids()
tf2automatic_bots = [x["steamid"] for x in requests.get('https://api.tf2automatic.com/v1/bots').json()['result']]

bots_to_add = bot_tf_ids + tf2automatic_bots
whitelist = list(requests.get('https://raw.githubusercontent.com/Apaar/backpack.tf-scripts-metadata/master/whitelist.json').json())
whitelist_add = []

for bot in bots_to_add:
    if bot not in whitelist:
        whitelist_add.append(str(bot))

print(json.dumps(whitelist_add))